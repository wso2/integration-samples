...
configurable string customMessage = "🛍️ New Shopify Order Received!<br>Order ID: {orderId}<br>Customer: {customerName}<br>Email: {customerEmail}<br>Items: {itemCount}<br>{items}<br>Subtotal: {currency} {subtotal}<br>Taxes: {currency} {taxes}<br>Shipping: {currency} {shipping}<br>Total: {currency} {totalPrice}<br>Shipping To: {shippingAddress}<br>Payment Status: {financialStatus}<br>Fulfillment Status: {fulfillmentStatus}<br>Created: {createdAt}";

// Minimum order price threshold - only send notifications for orders above this amount (default: 0 to send all)
configurable int minimumOrderPrice = 0;
