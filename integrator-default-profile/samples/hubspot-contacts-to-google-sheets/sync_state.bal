...
function getLastSyncTimestamp() returns string {
    // Check if there's a persisted timestamp in spreadsheet state sheet
    ...
    if sheetCheckpoint is string {
        ...
        log:printError(string `Warning: could not read checkpoint from sheet '${SYNC_STATE_SHEET}': ${sheetCheckpoint.message()}. Falling back to config.`);
    }
...
function saveLastSyncTimestamp(string timestamp) returns error? {
    ...
    log:printError(string `Warning: could not persist checkpoint to sheet '${SYNC_STATE_SHEET}': ${sheetWriteResult.message()}. Using in-memory checkpoint for this process.`);
